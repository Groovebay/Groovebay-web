...
const createStockReservationTransactions = async ({ tx, sdk, queryParams }) => {
  ...
  if (Object.keys(providerCart).length == 0) {
    return tx;
  } else {
    const additionalTransactionFns = Object.keys(providerCart).map(listingId => {
      const quantity = providerCart[listingId].quantity;
      // initiate
      const bodyParams = {
