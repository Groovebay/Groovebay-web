 ...
 * @param {string} address.countryCode - ISO 3166-1 alpha-2 country code (e.g., 'NL')
 * @param {string} [address.postalCode] - Postal code
 * @param {string} [address.city] - City name
 * @param {string} [address.houseNumber] - House number (required for NL addresses)
 ...
 */
const validate = async (address, validationType = null) => {
  ...
  try {
    ...
    throw new Error(error.response?.data?.message || error.message || 'Failed to validate address');
  }
};
